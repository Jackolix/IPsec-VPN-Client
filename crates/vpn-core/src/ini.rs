//! Minimal, defensive ini parser shared by the profile importers.
//!
...
//! profile file.
//!
//! Two dialects ride on this: NCP's `.ini` export (`[PROFILE1]`, `Key=Value`)
//! and Sophos/Cyberoam's `.tgb` (`[Phase 1]`, `Key = Value`, `#` comments,
//! and section names and keys that contain spaces or are bare IP addresses).
//! Both are covered by trimming keys and section names and treating `;`/`#`
//! lines as comments.
...
/// Strip one pair of surrounding double quotes, if present. Both dialects
/// quote values containing special characters — NCP's `Secret="..."` and
/// `.tgb`'s `Authentication = "..."`, each of which is the PSK.
...
mod tests {
    ...
    /// The `.tgb` dialect: `#` comments, spaces around `=`, spaces in section
    /// names, and a bare IP address as a key (`[Phase 1]` maps peer -> section).
    #[test]
    fn parses_tgb_dialect() {
        let doc = parse(
            "# written by VpnConf\n\
             [Phase 1]\n\
             192.168.2.2 = IPsecVPN-P1\n\
             [IPsecVPN-P1]\n\
             Authentication = \"s3cr3t\"\n\
             NATT_ENABLED = 1\n",
        )
        .unwrap();
        assert_eq!(
            doc.section("Phase 1").unwrap().get("192.168.2.2"),
            Some("IPsecVPN-P1")
        );
        assert_eq!(
            doc.section("IPsecVPN-P1").unwrap().get("Authentication"),
            Some("s3cr3t")
        );
    }

    /// Trailing whitespace is common in `.tgb` (`Retransmits = 5 `).
    #[test]
    fn trims_trailing_space_in_values() {
        let doc = parse("[A]\nDPD-interval = 60 \n").unwrap();
        assert_eq!(doc.section("A").unwrap().get("DPD-interval"), Some("60"));
    }
}
