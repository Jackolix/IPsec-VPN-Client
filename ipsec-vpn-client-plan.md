...
1. Imports NCP-style `.ini` VPN profiles (like the sample `EFA_MDT_42.ini`) and converts them into a working connection config.
...
- **Never** test against the profile's production gateway
  (`ITS5597555LCM010.itm-sourcing.net`) without explicit authorization; the
